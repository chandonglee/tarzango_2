...
class Ean_model extends CI_Model{
    ...
    $this->db->insert('pt_ean_booking',$data);
     ...
     function insert_booking_extra($insertdata){

          $data = array(
            'booking_id' => $insertdata['booking_id'],
            'hotel_type' => $insertdata['hotel_type'],
            'extra_data' => $insertdata['extra_data'],
          );

          $this->db->insert('pt_book_extra',$data);
   ...
   // $data['nums'] = $query->num_rows();
    ...
    function eanSignup_account($userdata) {
        ...
        $password = rand(6, 15);
        $data = array('accounts_email' => $userdata['email'],
                      'accounts_password' => sha1($password),
                      'accounts_type' => 'guest',
                      'ai_first_name' => $userdata['firstName'],
                      'ai_last_name' => $userdata['lastName'],
                      'ai_mobile' => $userdata['phone'],
                      'ai_address_1' => $userdata['address'],
                      'accounts_created_at' => $now, 
                      'ai_country' => $userdata['country'],
                      'ai_city' => $userdata['city'],
                      'ai_state' => $userdata['province'],
                      'ai_postal_code' => $userdata['postalcode'],
