...
// update menu order
		...
		public function group_booking_add(){
			...
			$this->load->helper('group_booking');
			...
			$this->form_validation->set_rules('g_contect_email', trans("094"), 'required|valid_email');
			$this->form_validation->set_rules('g_company_name', 'Enter valid name', 'trim|required');
			$this->form_validation->set_rules('check_in', 'Check in date ', 'trim|required');
			$this->form_validation->set_rules('check_out', 'Check out date ', 'trim|required');
			$this->form_validation->set_rules('g_attendees', 'No of attendees', 'trim|required');
			$this->form_validation->set_rules('rooms', 'Room', 'trim|required');
			$this->form_validation->set_rules('locality', 'City', 'trim|required');
			$this->form_validation->set_rules('administrative_area_level_1', 'State', 'trim|required');
			$this->form_validation->set_rules('g_place_type', 'Event type', 'required');
			$this->form_validation->set_rules('g_contect_name', 'Contact name ', 'required');
			$this->form_validation->set_rules('g_contect_no', 'Contact no', 'required');
			$this->form_validation->set_rules('sel_hotel_id', 'Select hotel', 'required');
			if ($this->form_validation->run() == FALSE) {
				...
				$attendees = $this->input->post('g_attendees');
				$aprx_rooms = $this->input->post('rooms');
				$city = $this->input->post('locality');
				$state = $this->input->post('administrative_area_level_1');
				$place_type = $this->input->post('g_place_type');
				$contect_name = $this->input->post('g_contect_name');
				$contect_no = $this->input->post('g_contect_no');
				$contect_email = $this->input->post('g_contect_email');
...
// enable main module
		...
		public function enable_main_module() {
				...
				if($modid == 'hotelbeds'){
					$this->modules_model->enable_main_module_hotelbeds($modid);
				}else{
...
// disable main module
		...
		public function disable_main_module() {
				...
				if($modid == 'hotelbeds'){
					$this->modules_model->disable_main_module_hotelbeds($modid);
				}else{
