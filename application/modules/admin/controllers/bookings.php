...
class Bookings extends MX_Controller {
    ...
    function index() {
		...
		$hotels = $this->bookings_model->getHotelsList();
		...
		$xcrud->join('booking_item', 'pt_hotels', 'hotel_id');
        $xcrud->order_by('booking_id', 'desc');
        $xcrud->columns('booking_id,booking_ref_no,pt_accounts.ai_first_name,booking_type,pt_hotels.hotel_title,booking_date,booking_total,booking_amount_paid,booking_remaining,booking_status');
        $xcrud->label('booking_id', 'ID')->label('booking_ref_no', 'Ref No.')->label('pt_accounts.ai_first_name', 'Customer')->label('booking_type', 'Module')->label('pt_hotels.hotel_title', 'Hotel Name')->label('booking_date', 'Date')->label('booking_total', 'Total')->label('booking_amount_paid', 'Paid')->label('booking_remaining', 'Remaining')->label('booking_status', 'Status');
        $xcrud->column_callback('booking_date', 'fmtDate');
        $xcrud->column_callback('booking_status', 'bookingStatusBtns');
        $xcrud->search_columns('booking_id,booking_ref_no,pt_accounts.ai_first_name,booking_type,pt_hotels.hotel_title,booking_status');
...
//resend invoice
    ...
    function edit($module, $id) {
        ...
        if (!empty ($module) && !empty ($id)) {
           ...
           // $this->data['extra_details'] = pt_get_extra_details($id, "2");
            /*echo json_encode( $this->data['bdetails']);
            exit();*/
            
            $extra_details = get_pickup_detail($id);
            ...
            if ($module == "hotels") {
                ...
                $this->data['userFullName'] = $this->data['bdetails']->userFullName;
                $this->data['accountEmail'] = $this->data['bdetails']->accountEmail;
            }
