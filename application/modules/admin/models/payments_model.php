...
exit;*/
    ...
    function getGatewayMsg($gateway,$invoicvars){
     ...
     function getGatewayMsg_paypal($gateway,$invoicvars){
     
        $this->settings = $this->settings_model->get_settings_data();

          $params = $this->getGatewayParams($gateway);
          $params['charset'] = "UTF-8";
          $params['invoiceid'] = $invoicvars['invoice_id'];
          $params['amount'] = $invoicvars['total'] + $invoicvars['tax'];
          $params['currency'] = $invoicvars['currency'];
          $params['description'] = $this->settings[0]->site_title;
         
          $this->load->library('gatewayslib');
          if(!empty($gateway)){
          $msg = $this->gatewayslib->gatewayMsg_tarzango($gateway,$params);
