...
// Add New supplier from fornt end register page
    function register_supplier() {
        ...
        $data = array(  'accounts_email' => $this->input->post('email'), 
                        ...
                        'accounts_is_admin' => 0, 
                        'ai_title' => $this->input->post('title'), 
...
//login
    function login_customer($username, $password) {
        ...
        if ($num > 0) {
            ...
            $member_data = check_is_member($user[0]->accounts_id);
            if($member_data){
                //echo "valisd";
                $this->session->set_userdata('is_member', '1');
            }else{
                $this->session->set_userdata('is_member', '0');
                //echo "invalisd";
            }
    ...
    function login_member_customer($username, $password) {
        $remember = $this->input->post('remember');
        $this->db->select('accounts_email,accounts_password,accounts_id,ai_first_name');
        $this->db->where('accounts_email', $username);
        $this->db->where('accounts_password', sha1($password));
    ...
    function login_facebook($userdata) {
        ...
        elseif (!empty ($userdata)) {
            $now = date('Y-m-d H:i:s');
            $insertdata = array(    'accounts_email' => $email, 
                                    'facebook_id' => $id, 
                                    'ai_first_name' => $fname, 
                                    'ai_last_name' => $lname, 
                                    'facebook_id' => $id, 
