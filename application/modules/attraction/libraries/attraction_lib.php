...
*/
    ...
    function apiCall_put($url,$post_data){
     ...
     function apiCall_delete($url,$post_data = null){
    ...
    function cancel_booking($booking_ref_no){
        
        $url = 'https://api.test.hotelbeds.com/activity-api/3.0/bookings/es/'.$booking_ref_no.'?cancellationFlag=CANCELLATION';
        $SIMULATION_data =  $this->apiCall_delete($url,$post_data);
        
        $SIMULATION_data = json_decode($SIMULATION_data);
        if(!$SIMULATION_data->errors){
            /*echo json_encode($SIMULATION_data);
            echo "<br>";
            echo "<br>";
            echo "<br>";
            $url = 'https://api.test.hotelbeds.com/activity-api/3.0/bookings/es/'.$booking_ref_no.'?cancellationFlag=SIMULATION';
            $SIMULATION_data =  $this->apiCall_delete($url,$post_data);
            echo json_encode($SIMULATION_data);*/
            $final_data = array(
                                'error' => 'no',
                                'msg' => json_encode($SIMULATION_data),
                                );
        }else{
            /*print_r($SIMULATION_data);*/
            $final_data = array(
                                'error' => 'yes',
                                'msg' => $SIMULATION_data->errors[0]->text,
                                );
        }
       /* echo "<pre>";
        print_r($SIMULATION_data);
