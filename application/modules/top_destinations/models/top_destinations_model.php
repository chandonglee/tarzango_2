...
class Top_destinations_model extends CI_Model {
        public $langdef;
...
// Call the Model constructor
		...
		function hotelGallery($slug){
          ...
          $this->db->select('himg_id as id,himg_top_destinations_id as itemid,himg_type as type,himg_image as image,image_title as image_title,himg_approved as approved');
          $this->db->where('himg_top_destinations_id',$slug);
          
          $imd_Data =  $this->db->get('pt_destinations_images')->result();
        ...
        function addPhotos($id,$filename){
		        ...
		        $imgorder = $this->db->get('pt_destinations_images')->num_rows();
		        $imgorder = $imgorder + 1;

				$approval = pt_admin_gallery_approve();
		    	$insdata = array(
                'himg_top_destinations_id' => $id,
                'himg_type' => 'slider',
                'himg_image' => $filename,
                'himg_approved' => $approval
                );
				$this->db->insert('pt_destinations_images', $insdata);
        ...
        // update hotel data
		function update_destinations_image_title($id,$image_title) {
			...
			$this->db->update('pt_destinations_images', $data);

		}

        // update hotel data
		function update_destinations($id) {

			$config['upload_path'] = 'uploads/images/dest_img/thumb_img/';
            ...
            $config['upload_path'] = 'uploads/images/dest_img/back_img/';
            $config['allowed_types'] = 'gif|jpg|png';
            $config['encrypt_name'] = TRUE;
            $this->load->library('upload', $config);
            $this->upload->initialize($config);
            ...
            if (!$this->upload->do_upload('back_img')) {
                $image_upload_data1 = array('is_upload' => false);
            } else { 
                $image_upload_data1['is_upload'] = true;
                $image_upload_data1['upload_data'] = $this->upload->data();
            }

            $config['upload_path'] = 'uploads/images/dest_img/detail_back_img/';
            $config['allowed_types'] = 'gif|jpg|png';
            $config['encrypt_name'] = TRUE;
            $this->load->library('upload', $config);
            $this->upload->initialize($config);
            $this->upload->set_allowed_types('*');
            $data['upload_data'] = '';
            if (!$this->upload->do_upload('detail_back_img')) {
                $image_upload_data2 = array('is_upload' => false);
            } else { 
                $image_upload_data2['is_upload'] = true;
                $image_upload_data2['upload_data'] = $this->upload->data();
            }
            ...
            $data = array(
			                ...
			                'to_see' => $this->input->post('to_see'),
			                'to_do' => $this->input->post('to_do'),
			                'to_stay' => $this->input->post('to_stay'),
			                );

				if($image_upload_data['is_upload'] == true){
	            	$thumb_img = $image_upload_data['upload_data']['file_name'];
	            	$data['thumb_img'] = $thumb_img;
	            }

	            if($image_upload_data1['is_upload'] == true){
	            	$back_img = $image_upload_data1['upload_data']['file_name'];
	            	$data['back_img'] = $back_img;
	            }
	            if($image_upload_data2['is_upload'] == true){
	            	$detail_back_img = $image_upload_data2['upload_data']['file_name'];
	            	$data['detail_back_img'] = $detail_back_img;
	            }

				$this->db->where('top_destinations_id', $id);
				$this->db->update('top_destinations', $data);

		}

		// Delete Hotel Images
		function delete_destinations_image($imgid, $imgname) {
				$this->db->where('himg_id', $imgid);
				$this->db->delete('pt_destinations_images');
               
                @ unlink(PT_DEST_SLIDER_THUMBS_UPLOAD . $imgname);
				@ unlink(PT_DEST_SLIDER_UPLOAD . $imgname);

		}

		// get number of photos of hotel
		function photos_count($hotelid) {
				$this->db->where('himg_top_destinations_id', $hotelid);
				return $this->db->get('pt_destinations_images')->num_rows();
		}

		// add hotel data
		function add_destinations($user = null) {
			if(empty($user)){
				$user = 1;
			}
            ...
            if($image_upload_data2['is_upload'] == true){
            	$detail_back_img = $image_upload_data1['upload_data']['file_name'];
            }else{
            	$detail_back_img = '';
            }
				...
				$data = array(
			                ...
			                'thumb_img' => $thumb_img,
			                'back_img' => $back_img,
			                'detail_back_img' => $detail_back_img,
						    'latitude' => $this->input->post('latitude'),
			                'longitude' => $this->input->post('longitude'),
			                'to_see' => $this->input->post('to_see'),
			                'to_do' => $this->input->post('to_do'),
				...
				$this->db->insert('top_destinations', $data);
				$top_destinations_id = $this->db->insert_id();
				
				return $top_destinations_id;
		}


		// get all data of single hotel by slug
		function get_destinations_data($hotelslug) {

				$this->db->select('*');
				$this->db->where('top_destinations_id', $hotelslug);
