...
// if($chk == "Yes"){
		...
		function is_main_module_enabled_hotel_beds($module) {
				$this->_ci->db->select('pt_hotel_beds_id');
				$this->_ci->db->where('is_avalible', '1');
				$this->_ci->db->where('pt_hotel_beds_id', '1');
				$rows = $this->_ci->db->get('pt_hotel_beds')->num_rows();
				if ($rows > 0) {
						return true;
				}
				else {
